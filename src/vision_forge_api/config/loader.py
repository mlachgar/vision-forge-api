...
class ConfigLoader:
    ...
    def load_settings(self) -> SettingsConfig:
        return SettingsConfig.model_validate(
            _read_yaml(self.config_dir / "settings.yaml")
        )

    def load_tag_sets(self) -> TagSetsConfig:
        return TagSetsConfig.model_validate(
            _read_yaml(self.config_dir / "tag_sets.yaml")
        )

    def load_profiles(self) -> ProfilesConfig:
        return ProfilesConfig.model_validate(
            _read_yaml(self.config_dir / "profiles.yaml")
        )

    def load_prompts(self) -> PromptsConfig:
        return PromptsConfig.model_validate(
            _read_yaml(self.config_dir / "prompts.yaml")
        )
