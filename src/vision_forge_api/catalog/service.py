...
from ..config.schema import Profile, ProfilesConfig, PromptsConfig, TagPrompt, TagSet, TagSetsConfig
...
class TagCatalog:
    ...
    def __init__(
        ...
        self._tag_sets: Mapping[str, TagSet] = {ts.name: ts for ts in tag_sets_config.tag_sets}
        if len(self._tag_sets) != len(tag_sets_config.tag_sets):
            raise ValueError("Duplicate tag set names are not allowed")
        self._profiles: Mapping[str, Profile] = {profile.name: profile for profile in profiles_config.profiles}
        ...
        self._prompts: Mapping[str, tuple[TagPrompt, ...]] = {
            entry.canonical_tag: tuple(entry.prompts) for entry in prompts_config.prompts
        }
        ...
        for profile in self._profiles.values():
            for tag_set_name in profile.tag_sets:
                if tag_set_name not in self._tag_sets:
                    raise ValueError(f"Profile '{profile.name}' references unknown tag set '{tag_set_name}'")
    ...
    def profile_detail(self, profile_name: str) -> ProfileDetail:
        ...
        return ProfileDetail(profile=profile, tag_sets=tag_sets, canonical_tags=canonical_tags)
